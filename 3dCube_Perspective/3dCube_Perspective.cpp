...
int main()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, arraySize*3*sizeof(GLfloat), vertexPositions1, GL_STATIC_DRAW);// push the vextex data into the VBO in video memory
	...
	while (window.isOpen())
    {
		...
		glDrawArrays(GL_TRIANGLES, 0, arraySize);	// draw  object 
